# Final Verification Script - Memastikan rebrand Arah Berita selesai dengan baik

$WorkspaceRoot = "c:\KULIAH\MAGANG\Magang di Perhutani\Arah Berita"

Write-Host "========== FINAL VERIFICATION - ARAH BERITA REBRAND ==========" -ForegroundColor Cyan
Write-Host ""
...
$oldBrandingPatterns = @("Warta Janten", "Warta janten", "wartajanten", "WartaJanten")
...
# 2. Check for legacy brand image references
Write-Host "2. Checking for legacy brand image references..." -ForegroundColor Yellow
$logoFound = Get-ChildItem -Path $WorkspaceRoot -Recurse -Include "*.html" -File |
    Select-String -Pattern 'img[^\n>]*logo\.(png|svg|jpg)|(logo\.(png|svg|jpg))[^\n<]*img' -ErrorAction SilentlyContinue |
    Where-Object { $_.Path -notlike "*\.bak.*" }

if ($logoFound) {
    $stats["Legacy logo refs"] = $logoFound.Count
    Write-Host "   ⚠️  Found $($logoFound.Count) legacy logo image references" -ForegroundColor Yellow
...
$newColors = @("#1D4ED8", "#1E3A8A", "#7F2F4F")
$colorsFound = 0
...
Write-Host "4. Checking for new branding 'Arah Berita'..." -ForegroundColor Yellow
$newBrandingFound = Get-ChildItem -Path $WorkspaceRoot -Recurse -Include "*.html" -File |
    Select-String -Pattern "Arah Berita|ArahBerita|arahberita" -ErrorAction SilentlyContinue |
    Where-Object { $_.Path -notlike "*\.bak.*" } | 
    Measure-Object

if ($newBrandingFound.Count -gt 0) {
    Write-Host "   ✅ Found 'Arah Berita' branding in $($newBrandingFound.Count) places" -ForegroundColor Green
} else {
    Write-Host "   ⚠️  No 'Arah Berita' branding found!" -ForegroundColor Yellow
    $issues += @{ Type = "Missing"; File = "All"; Reason = "No Arah Berita branding found" }
}
...
foreach ($pkg in $pkgFiles) {
    ...
    if ($content -match '"name"\s*:\s*"arahberita') {
        $pkgOK++
...
Write-Host ""
Write-Host "Rebrand Arah Berita SELESAI ✅" -ForegroundColor Green
